...
static inline T *swift_slowCreate(Args &&... args) {
  auto result = reinterpret_cast<T *>(swift_slowAlloc(sizeof(T), alignof(T) - 1));
  ::new (result) T(std::forward<Args>(args)...);
  return result;
}

/// Destruct and deallocate an instance of type \c T.result
///
/// \param ptr A pointer to an instance of type \c T previously created with a
/// 	call to \c swift_slowCreate().
///
...
/// from \c swift_slowCreate() is undefined.
template <typename T>
static inline void swift_slowDestroy(T *ptr) {
  if (ptr) {
  	ptr->~T();
